...
% Fill the center pixel matrix C.图像中所有可以编码的像素
...
for i = 1:neighbors  %每个近邻采样点处理模式相同，且对于其他像素后面程序用了“批”处理方式（相当于模板平移操作）！
  y = spoints(i,1)+origy;
  x = spoints(i,2)+origx;
  ...
  if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
    ...
    D = N >= C; % 与中心像素比较，结果为0或者1，这里是矩阵并行运算
  else
    % Interpolation needed, use double type images 为什么？？？？？double
    ty = y - fy;
    tx = x - fx;
...
if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    ...
    if (strcmp(mode,'nh'))
        result=result/sum(result); %l1归一化
    end
else
